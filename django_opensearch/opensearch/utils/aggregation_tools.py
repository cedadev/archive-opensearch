...
from django_opensearch.settings import THREDDS_HOST
import requests
import xmltodict
...
def get_thredds_aggregation(id, format='xml'):
    return f'http://{THREDDS_HOST}/thredds/datasets/{id}.{format}?dataset={id}'
...
def get_aggregation_capabilities(id):
    ...
    for service in data['catalog']['service']:
        services.append({
            'title': service['@name'],
            'href': f'http://{THREDDS_HOST}{service["@base"]}{id}{SUFFIX_MAP.get(service["@name"],"")}'
        })
